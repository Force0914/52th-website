  ...
  setup(props, context) {
    ...
    const refresh = () => {
      ...
      if(input.value.avatar) {
        const ratio = 280 / input.value.avatar.height
        ctx.save()
        roundedImage(499, 173, input.value.avatar.width * ratio, 280, 0)
        ctx.clip()
        ctx.drawImage(input.value.avatar, 499, 173, input.value.avatar.width * ratio, 280)
        ctx.restore()
      }
      ...
      colorText(input.value.joinDate, 200, 230, 'bold #393939', '40px ' + fonts, 'left')
      colorText(input.value.location, 200, 278, 'bold #393939', '40px ' + fonts, 'left')
      colorText(pubDateText.value, 200, 475, 'bold #393939', '35px ' + numfonts, 'left')
      colorText(input.value.name, 200, 326, 'bold #393939', '40px ' + fonts, 'left')
    }
