<script setup>
import { computed } from '@vue/reactivity';
import { ref, reactive, onMounted, provide, shallowRef } from 'vue'

const search = ref("");
const classList = reactive([]);
const studentList = reactive([]);
const dialog = reactive({
    addClass: false,
    addStudent: false
});
const dialogData = reactive({
    "class":{
        name: ""
    },
    "student":{
        avathar: "",
        fullname: {
            last: "",
            first: ""
        },
        email: [],
        phone: [],
        class: ""
    }
});

const totalStudent = computed(() => {
    return studentList.length;
});

const load = async() => {
    let db = await getDB();
    let transaction = db.transaction("class", "readwrite");
    let objectStore = transaction.objectStore("class");
    let request = objectStore.get("class");
    let stransaction = db.transaction("student", "readwrite");
    let sobjectStore = stransaction.objectStore("student");
    let srequest = sobjectStore.get("student");
    request.onerror = (event) => {
        console.log("error");
    };
    srequest.onerror = (event) => {
        console.log("error");
    };
    srequest.onsuccess = (event) => {
        let data = {};
        if (request.result) {
            data = JSON.parse(request.result);
        }

        studentList.splice(0, studentList.length, data);
    };
    request.onsuccess = (event) => {
        let data = {
            classList: []
        };
        if (request.result) {
            data = JSON.parse(request.result);
        }

        classList.splice(0, classList.length, ...data.classList.map((c) => {
            return {
                name: c,
                total: 0
            }
        }));
    };
}

const getDB = () => {
    return new Promise((resolve) => {
        let request = window.indexedDB.open("52", 1);
        request.onupgradeneeded = function (event) {
            let db = request.result;
            let objectStore = db.createObjectStore("class");
            let studentStore = db.createObjectStore("student");
        };
        request.onsuccess = function (event) {
            let db = request.result;
            resolve(db);
        };
    });
}

const save = async() => {
    let data = JSON.stringify({
        classList: classList.map((c) => {
            return c.name;
        })
    });
    let sdata = JSON.stringify(studentList);
    let db = await getDB();
    let stransaction = db.transaction("student", "readwrite");
    let sobjectStore = stransaction.objectStore("student");
    let srequest = sobjectStore.put(data,"student");
    let transaction = db.transaction("class", "readwrite");
    let objectStore = transaction.objectStore("class");
    let request = objectStore.put(data,"class");
}

const openAddClass = () => {
    dialogData.class.name = "";

    dialog.addClass = true;
}

const openAddStudent = () => {
    dialog.addStudent = true;
}

const addClass = async() => {
    if (dialogData.class.name.trim() == "") {
        return
    }
    classList.push({
        name: dialogData.class.name,
        total: 0
    });
    dialog.addClass = false;
    await save();
}

const openAvatarUpload = () =>{
    document.getElementById("avatar_upload").click();
}

onMounted(() => {
    load();
});

</script>

<template>
    <header id="header">
        <h1 id="logo">
            學生資料
        </h1>
        <form id="searchForm" @submit.prevent>
            <input type="text" name="search" v-model="search" placeholder="搜尋" autocomplete="off">
        </form>
    </header>
    <div class="flex">
        <aside id="aside">
            <button id="addStudent" @click="openAddStudent">建立學生</button>
            <div>
                <ul class="list" id="studentList">
                    <li class="item current">
                        所有學生
                        <span class="num">{{totalStudent}}</span>
                    </li>
                </ul>
            </div>
            <div>
                <div class="side-title">班級</div>
                <ul class="list" id="classList">
                    <li class="item" v-for="c of classList">
                        {{c.name}}
                        <span class="num">{{c.total}}</span>
                    </li>
                    <li class="item" id="addClass" @click="openAddClass">
                        建立班級
                    </li>
                </ul>
            </div>
            <div>
                <ul class="list">
                    <li class="item">
                        垃圾桶
                    </li>
                </ul>
            </div>
        </aside>
        <main id="main">
            <table class="contacts" v-if="totalStudent != 0">
                <thead>
                    <tr class="contact">
                        <th>姓名</th>
                        <th>學號</th>
                        <th>電子郵件</th>
                        <th>電話號碼</th>
                        <th>班級</th>
                    </tr>
                </thead>
                <tbody>
                    <tr class="contact" v-for="s of totalStudent" :key="s.id">
                        <td class="name-group">
                            <img src="data:image/png;base64," class="avator">
                            <p class="fullname">許凱俊</p>
                        </td>
                        <td class="student_id">914631</td>
                        <td class="email">kaijun0914@icloud.com</td>
                        <td class="phone">0966897121</td>
                        <td class="class"><span class="badge">訊二甲</span></td>
                    </tr>
                </tbody>
            </table>
            <div class="message" v-else>
                <p>目前還沒有任何學生</p>
            </div>
        </main>
        <Transition>
            <div class="dialog" id="dialog" v-if="dialog.addClass || dialog.addStudent">
                <div v-if="dialog.addClass">
                    <h2 class="title">建立班級</h2>
                    <form class="newClass">
                        <input type="text" class="newinput" placeholder="班級名稱" name="name" v-model="dialogData.class.name" autocomplete="off">
                        <div class="flex">
                            <button type="button" class="close" style="margin: 12px;"
                                @click="dialog.addClass = false">取消</button>
                            <button type="button" class="submit" style="margin: 12px;" @click="addClass"
                                :disabled="dialogData.class.name.trim() == ''">儲存</button>
                        </div>
                    </form>
                </div>
                <div v-if="dialog.addStudent">
                    <form class="newContact">
                    <h2 class="title">建立學生</h2>
                        <hr>
                        <div class="form">
                            <div class="name">
                                <input type="text" class="newinput" placeholder="姓氏" name="last_name">
                                <input type="text" class="newinput" placeholder="名字" name="first_name" required>
                                <img src="data:image/png;base64," class="avatar_preview" @click="openAvatarUpload">
                                <input type="file" id="avatar_upload" class="avatar" accept="image/*" style="display: none;">
                            </div>
                            <input type="email" name="email[]" class="newinput" placeholder="電子郵件">
                            <input type="tel" name="phone[]" class="newinput" placeholder="電話">
                            <select class="class newselect" name="class">
                                <option value="" selected>請選擇班級</option>
                                <option v-for="c in classList" :value="c.id">{{c.name}}</option>
                            </select>
                            <textarea name="note" cols="30" rows="10" class="newtextarea" placeholder="備註"></textarea>
                        </div>
                        <hr>
                    <div class="flex">
                        <button type="button" class="close" style="margin: 12px;"
                            @click="dialog.addStudent = false">取消</button>
                        <button type="submit" class="submit" style="margin: 12px;" @click="addStudent">儲存</button>
                    </div>
                    </form>
                </div>
            </div>
        </Transition>
    </div>
</template>

<style>
    @import './assets/base.css';
</style>